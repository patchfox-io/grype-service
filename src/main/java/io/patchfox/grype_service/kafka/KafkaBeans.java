package io.patchfox.grype_service.kafka;
...
import io.patchfox.grype_service.controllers.GrypeController;
import io.patchfox.grype_service.controllers.HealthCheckController;
import io.patchfox.grype_service.controllers.RestInfoController;
import io.patchfox.grype_service.services.RestInfoService;
...
public class KafkaBeans {
    ...
    private ApiResponse invokeMethod(
            ...
            ApiRequest apiRequest // <-- this is lazy AF 
        ...
        switch(restSignature) {
            ...
            case RestInfoController.GET_REST_INFO_SIGNATURE:
                ...
                rv = re.getBody();
                break;
            case GrypeController.POST_GRYPE_SIGNATURE:
                try {
                    //var payload = (PackageWrapper)ApiDataHelpers.getPojoFromDataMap(apiRequest.getData(), PackageWrapper.class);
                    var datasourceEventRecordId = Long.valueOf(apiRequest.getQueryStringParameters().get("datasourceEventRecordId"));
                    //log.debug("payload is: {}", payload);

                    var altTxidOptional = Optional.empty();
                    if (apiRequest.getQueryStringParameters().containsKey("altTxid")) {
                        var altTxid = UUID.fromString(apiRequest.getQueryStringParameters().get("altTxid"));
                        altTxidOptional = Optional.of(altTxid);
                    }


                    re = (ResponseEntity<ApiResponse>)beanMethod.invoke(bean, txid, requestReceivedAt, datasourceEventRecordId, altTxidOptional);
                    rv = re.getBody(); 

                    // if (apiRequest.getQueryStringParameters().containsKey("recordResults")) {
                    //     log.info("found query string parameter for 'recordResults'. using that instead of default.");
                    //     var recordResultsAsString = apiRequest.getQueryStringParameters().get("recordResults");
                    //     var recordResults = Boolean.parseBoolean(recordResultsAsString);
                    //     re = (ResponseEntity<ApiResponse>)beanMethod.invoke(bean, txid, requestReceivedAt, datasourceEventRecordId, recordResults, altTxidOptional);
                    //     rv = re.getBody();                       
                    // } else {
                    //     re = (ResponseEntity<ApiResponse>)beanMethod.invoke(bean, txid, requestReceivedAt, datasourceEventRecordId, altTxidOptional);
                    //     rv = re.getBody(); 
